...
const fs = require("fs");
const { request, sha1URL, sha1, sha1Buffer } = require("./app/utils.js");
const { bencodeToBuffer } = require("./app/encode-buffer.js");
const { decodeBufferBencode } = require("./app/decode-buffer.js");
const { createHandshake } = require("./app/handshake.js");
...
async function main() {
  ...
  try {
    if (command === "decode") {
      ...
      console.log(peerValues.peers.toString("hex"));
      ...
      createHandshake({ peerAddress: ip, peerPort: port, infoHash, peerId });
    } else {
